...
function toggleTheme() {
  ...
  btn.innerHTML = '<span class="material-symbols-rounded">' + (next === 'dark' ? 'dark_mode' : 'light_mode') + '</span>';
...
(function initTheme() {
  ...
  btn.innerHTML = '<span class="material-symbols-rounded">' + (theme === 'dark' ? 'dark_mode' : 'light_mode') + '</span>';
})();
...
document.querySelectorAll('.card, .btn, .tab-label, .accordion-item, .pricing-card, .testimonial').forEach(el => {
...
window.addEventListener('scroll', () => {
  ...
  if (hero && y < window.innerHeight) {
    ...
    hero.style.transform = `translateY(${p * 100}px) scale(${1 - p * 0.05})`;
  }
}, { passive: true });
