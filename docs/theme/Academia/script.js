function toggleTheme() {
  ...
  document.getElementById('themeBtn').querySelector('.material-symbols-outlined').textContent = next === 'dark' ? 'dark_mode' : 'light_mode';
  localStorage.setItem('theme', next);
}
(function initTheme() {
  const saved = localStorage.getItem('theme') || (window.matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark');
  document.body.dataset.theme = saved;
  document.getElementById('themeBtn').querySelector('.material-symbols-outlined').textContent = saved === 'dark' ? 'dark_mode' : 'light_mode';
})();
...
console.log('Nexus — Academia volume loaded');