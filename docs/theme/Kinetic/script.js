...
window.addEventListener('scroll', () => {
  ...
  // Hero parallax: scale 1.0→1.2, opacity 1.0→0 over first 20% of scroll
  const hero = document.querySelector('.hero-content');
  if (hero) {
    const scrollY = window.scrollY;
    const heroHeight = window.innerHeight;
    const progress = Math.min(scrollY / (heroHeight * 0.2), 1);
...
console.log('Nexus Kinetic landing page loaded ✅');