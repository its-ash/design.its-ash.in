...
function toggleTheme() {
  ...
  document.querySelector('#themeBtn .material-symbols-outlined').textContent = next === 'dark' ? 'dark_mode' : 'light_mode';
  document.getElementById('themeBtn').setAttribute('aria-pressed', next === 'light');
...
(function initTheme() {
  ...
  if (saved) {
    ...
    document.querySelector('#themeBtn .material-symbols-outlined').textContent = saved === 'dark' ? 'dark_mode' : 'light_mode';
    document.getElementById('themeBtn').setAttribute('aria-pressed', saved === 'light');
  }
})();
...
contactForm.addEventListener('submit', e => {
  ...
  set('cName', 'errName', '[ERR] name is required');
  const email = document.getElementById('cEmail');
  const emailErr = document.getElementById('errEmail');
  if (!email.value) { emailErr.textContent = '[ERR] email is required'; email.classList.add('invalid'); valid = false; }
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) { emailErr.textContent = '[ERR] invalid email format'; email.classList.add('invalid'); valid = false; }
  ...
  if (phone.value && !/^[\d\s\+\-\(\)]{7,}$/.test(phone.value)) { phoneErr.textContent = '[ERR] invalid phone number'; phone.classList.add('invalid'); valid = false; }
  else { phoneErr.textContent = ''; phone.classList.remove('invalid'); }
  set('cSubject', 'errSubject', '[ERR] please select a subject');
  const msg = document.getElementById('cMessage');
  const msgErr = document.getElementById('errMessage');
  if (!msg.value) { msgErr.textContent = '[ERR] message is required'; msg.classList.add('invalid'); valid = false; }
  else if (msg.value.length < 10) { msgErr.textContent = '[ERR] message must be at least 10 characters'; msg.classList.add('invalid'); valid = false; }
  else { msgErr.textContent = ''; msg.classList.remove('invalid'); msg.classList.add('valid'); }
  set('cTerms', 'errTerms', '[ERR] you must agree to continue');
  if (valid) {
...
console.log('NEXUS // terminal CLI loaded [OK]');