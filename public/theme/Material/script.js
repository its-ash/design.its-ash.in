...
function toggleTheme() {
  ...
  if (span) span.textContent = next === 'dark' ? 'dark_mode' : 'light_mode';
...
(function initTheme() {
  ...
  const btn = document.getElementById('themeBtn');
  const span = btn.querySelector('.material-symbols-rounded');
  if (span) span.textContent = theme === 'dark' ? 'dark_mode' : 'light_mode';
...
function showToast(msg) {
  ...
  t.className = 'toast';
  t.innerHTML = '<span class="material-symbols-rounded">check_circle</span><span>' + msg + '</span>';
  c.appendChild(t);
...
contactForm.addEventListener('submit', e => {
  ...
  const set = (id, errId, msg) => {
    ...
    if (!el.value || (el.type === 'checkbox' && !el.checked)) { err.textContent = msg; el.classList.add('invalid'); el.classList.remove('valid'); valid = false; }
  ...
  if (!email.value) { emailErr.textContent = 'Email is required'; email.classList.add('invalid'); email.classList.remove('valid'); valid = false; }
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) { emailErr.textContent = 'Invalid email format'; email.classList.add('invalid'); email.classList.remove('valid'); valid = false; }
  ...
  if (phone.value && !/^[\d\s\+\-\(\)]{7,}$/.test(phone.value)) { phoneErr.textContent = 'Invalid phone number'; phone.classList.add('invalid'); phone.classList.remove('valid'); valid = false; }
  else { phoneErr.textContent = ''; phone.classList.remove('invalid'); if (phone.value) phone.classList.add('valid'); }
  ...
  if (!msg.value) { msgErr.textContent = 'Message is required'; msg.classList.add('invalid'); msg.classList.remove('valid'); valid = false; }
  else if (msg.value.length < 10) { msgErr.textContent = 'Message must be at least 10 characters'; msg.classList.add('invalid'); msg.classList.remove('valid'); valid = false; }
