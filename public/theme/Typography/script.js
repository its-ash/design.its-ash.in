...
function toggleTheme() {
  ...
  document.getElementById('themeBtn').innerHTML = '<span class="material-symbols-outlined">' + (next === 'dark' ? 'dark_mode' : 'light_mode') + '</span>';
  localStorage.setItem('theme', next);
}
(function initTheme() {
  const saved = localStorage.getItem('theme') || (matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark');
  document.body.dataset.theme = saved;
  document.getElementById('themeBtn').innerHTML = '<span class="material-symbols-outlined">' + (saved === 'dark' ? 'dark_mode' : 'light_mode') + '</span>';
})();
